...
function writeDb(data){ ensureStorage(); const normalized=normalizeDb(data); normalized.version=2.22; const temp=DB_FILE+'.tmp'; fs.writeFileSync(temp,JSON.stringify(normalized,null,2),'utf8'); fs.renameSync(temp,DB_FILE); return normalized; }
...
app.get('/api/health',(_req,res)=>res.json({ok:true,version:'2.2.2'}));
...
ensureStorage();
app.listen(PORT,HOST,()=>{ console.log('===================================================='); console.log('Sistem Bilgisayar Teknik Servis Pro v2.2.2 çalışıyor'); console.log(`Bu bilgisayardan: http://localhost:${PORT}`); console.log(`Yerel ağdan: http://[BILGISAYAR-IP]:${PORT}`); console.log('===================================================='); });
