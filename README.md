# Teknik Servis Pro v2.3.1 — Python Edition
...
Kurulum güncelleme sırasında çalışan v2.3.1+ sunucuya yerel kapanış isteği yollar; eski v2.3.0 `TeknikServisPro.exe` sürecini de kapatır. Böylece `MoveFile failed; code 5` dosya kilidi hatası azaltılır. Mevcut `db.json` güncellemede korunur.
...
4. Inno Setup `TeknikServisPro_v2_3_1_Setup.exe` dosyasını üretir.
5. Build tamamlanınca `TeknikServisPro-v2.3.1-Python-Windows-Setup` artifact'ını indirin.
